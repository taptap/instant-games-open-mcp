...
export function resolvePathSafe(
  ...
  if (inputType === 'empty') {
    if (allowEmpty) {
      // 允许空路径，返回基础路径
      logger.info(`[PathResolver] Empty path allowed, using base path: ${basePath}`).catch(() => {});
      return {
        success: true,
  ...
  if (isProxyMode && inputType === 'absolute') {
    logger.warning(`[PathResolver] Absolute path rejected in Proxy mode: ${trimmedInput}`).catch(() => {});
    return {
      success: false,
      inputPath: trimmedInput,
      inputType,
  ...
  // 4. 计算最终路径
  const resolvedPath = inputType === 'absolute'
    ? trimmedInput
    : path.join(basePath, trimmedInput);
