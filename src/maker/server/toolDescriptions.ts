...
const MAKER_REMOTE_PROXY_PUBLIC_DESCRIPTIONS: Readonly<Record<string, string>> = {
  ...
  achievement: [
    'TapTap V4 成就管理 MCP 工具。已接入全部 18 个底层 Agent API，对外提供 19 个高层 op；部分 op 会组合或复用同一个底层接口。',
    '身份参数 app_id、developer_id、client_id 均从可信项目配置和服务端环境获取，不允许调用方传入。普通成就对外使用 achievement_id（对应后端 openid），managementId 仅保存在 .project/achievements.lock.json。白金成就是应用级单例，固定 is_hidden=false、blocks_platinum=false、total_step=1；传入 is_hidden 会报错，achievement_id、blocks_platinum、total_steps 会被忽略。',
    '操作与参数：op="sync_achievements" 无业务参数，同步全部普通成就和白金成就并刷新 lock。op="get_achievement" 必填 achievement_id。op="create_achievement" 必填 achievement_id、total_steps，可选 title、sub_title、image_url、is_hidden、blocks_platinum。op="update_achievement" 必填 achievement_id，并至少提供一个实际变化字段。op="delete_achievement" 必填 achievement_id。op="set_achievement_order" 必填 achievement_id、list_order。op="check_publish_achievements" 无业务参数。op="publish_achievements" 无业务参数，执行发布检查后发布普通成就。op="get_platinum_achievement" 无业务参数。op="create_platinum_achievement" 可选 title、sub_title、image_url，至少需要 10 个符合条件的普通成就。op="update_platinum_achievement" 至少提供 title、sub_title、image_url 之一且值必须发生变化。op="delete_platinum_achievement" 无业务参数。op="check_publish_platinum_achievement" 无业务参数。op="publish_platinum_achievement" 无业务参数，执行发布检查后发布白金成就。op="cancel_platinum_achievement_audit" 无业务参数，仅适用于审核中的白金成就。op="add_achievement_test_users" 必填 user_ids，数量 1-100。op="reset_achievement" 必填 achievement_id，仅重置普通成就玩家数据。op="list_achievement_test_users" 必填 page_num、page_size，可选 user_id（允许 0）。op="reset_achievement_test_user" 必填 user_id，必须大于 0。',
    '所有定义写操作在远端成功后都会尝试同步最新数据并刷新 lock。若返回 success=true、remote_applied=true 且 lock_sync.synced=false，平台写入已成功，只再调用 sync_achievements 恢复，不要重放原来的写操作。本地代理原样转发参数和返回，不读写本机 .project。',
  ].join(' '),
};
