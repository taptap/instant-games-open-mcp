...
function createMakerAgentsPolicyBody(): string {
  return [
    ...
    '- For achievement requests, call `achievement` on the bound project and follow its schema.',
    '- The local proxy forwards arguments and results unchanged. Remote maker-tools reads and writes',
    '  the remote workspace `.project/` files. Do not read or edit the local `.project/` copy for',
    '  achievement identity or `achievements.lock.json`.',
    '- If `success=true`, `remote_applied=true`, and `lock_sync.synced=false`, call',
    '  `sync_achievements` only and do not replay the original write.',
    '',
