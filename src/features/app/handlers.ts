...
export async function listDevelopersAndApps(context: HandlerContext): Promise<string> {
  try {
    const result = await getAllDevelopersAndApps(context.macToken);
...
export async function selectApp(
  ...
  try {
    const result = await selectAppApi(args.developer_id, args.app_id, context.projectPath, context.macToken);
