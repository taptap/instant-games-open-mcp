...
> 广告状态和广告位 ID 会由 MCP 自动查询，开发者无需提供或手工填写广告位 ID。
> 本 MCP 的广告流程仅适用于 TapTap 小游戏/H5；Maker/UrhoX 项目应使用独立的 Maker MCP，两者不得混用。
> 状态已生效只表示可以生成接入代码，实际播放仍需在 TapTap 小游戏/H5 宿主环境中验证。
