...
本地 Lua 诊断最终依赖 `maker-lua-lsp`，其安装包自带 `luac`。本功能负责先准备
Python 运行时，然后触发 `maker-lua-lsp` 安装和 IDE 配置；LSP 安装失败只提示错误，
不阻塞远端构建主流程。
...
新增 Maker Python runtime 和 Lua LSP runtime 模块：
...
- tgz 大小：约 282 KB
- 包内不包含 uv、Python runtime 或 maker-lua-lsp；首次 `python setup` 时实时下载并缓存。
