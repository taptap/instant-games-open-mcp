...
指纹。远端断线由 manager 自动恢复，无需重新安装或重启本地 MCP。包版本或本地工具白名单
改变后，需 Reconnect 本地 MCP；若 AI 客户端不支持 `tools/list_changed`，同样需要手动
Reconnect 以刷新工具列表。runtime-log watcher 仍使用独立的 polling connection lifecycle。
