...
- `maker_status`：统一输出本地 Maker 状态、Git 前置条件和初始化引导；如果发现本地已有 PAT 但缺少 TapTap token，会自动尝试获取；如果当前目录未绑定，会自动列出 app，不需要用户额外要求。但如果当前目录明显是 AI 客户端对话临时目录（如 `xdt-maker/dialogues/...`），则不列 app、不引导 clone，改为提示 Agent 选择客户端附加工作目录并重新调用 `maker_status(target_dir="<项目目录>")`。
...
- 如果上一次提交已生成本地 commit 但 push 失败，后续 `maker_submit_current_directory` 或构建自动提交流程会识别本地 ahead commit，继续执行 push；不会因为工作区没有未提交文件而跳过。
- 构建前本地改动检查会忽略 `.gitignore` 和 `.maker-mcp/` 的本地辅助变化；这些变化不应单独触发“本地改动未提交”的提醒。
...
`maker_status` 会输出已随包内置的 skill 名称和文档路径：`taptap-maker-local`、`taptap-maker-dev-kit-guide` 与 `update-taptap-mcp`。除此之外不做编辑器安装引导。Maker 操作目标是用户当前项目目录；若 MCP 进程 cwd 是临时对话目录，Agent 应优先检查客户端附加工作目录：只有一个附加目录时直接作为 `target_dir`，多个附加目录时让用户选择哪个是 Maker 项目目录，避免把已绑定项目误判为未绑定。
...
如果 clone 失败，工具会返回 `partial_state`，说明目标目录是否已经初始化 `.git`、是否已经绑定 `.maker-mcp/config.json`、dev kit 是否已落盘、以及是否可以直接重试。skill 负责把这些状态解释给用户；不要自动删除半初始化文件。对新手用户，连续失败时优先建议换一个全新的独立目录重新 clone。
...
这种状态下，`maker_status` 会显示 Maker config root 和 Git root 不一致；`build` 和
`submit` 会停止，避免把外层大仓库当成 Maker 项目提交或推送。初始化前，skill 应提醒
用户优先选择独立目录；如果用户确认继续，clone 工具会在目标目录内创建独立 Maker Git
仓库，不复用父级 Git 仓库。
...
- 如果 commit 已完成但 push 失败，工具必须返回 `failed_after_commit`、commit hash、git ahead 状态、失败阶段、exit code、stderr/stdout、错误分类、`push_recovery` 和下一步建议。
- `push_recovery` 必须明确告诉 AI：本地提交已保留但未推送；不要手动执行通用 `git push`；直接重试 `maker_submit_current_directory`，或在构建重试时调用 `maker_build_current_directory(submit_local_changes_before_build=true)`。
