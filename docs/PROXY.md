...
| `_client_session_id` | `string`                 | 业务/编辑器会话 ID                 | ❌   |
...
```json
{
  ...
  "params": {
    ...
    "arguments": {
      ...
      "_user_id": "user_12345",
      "_client_session_id": "client_session_abc",
...
- `X-TapTap-Client-Session-Id`: 业务/编辑器会话 ID（可选，Server 侧解析为 `ctx.clientSessionId`）
...
```json
{
  ...
  "tenant": {
    "project_path": ".",
    "user_id": "user123",
    "client_session_id": "client-session-789",
...
- `tenant.client_session_id` - 业务/编辑器会话标识符（可选，透传为 `ctx.clientSessionId`）
...
interface ProxyConfig {
  ...
  tenant: {
    ...
    client_session_id?: string; // 业务/编辑器会话 ID
...
function generateProxyConfig(user: User, project: Project, macToken: MacToken): string {
  const config = {
    ...
    tenant: {
      ...
      client_session_id: project.clientSessionId, // 业务/编辑器会话 ID
