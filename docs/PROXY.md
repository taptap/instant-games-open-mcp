...
通用 Proxy 的默认工具超时恢复为 5 分钟。此前依赖隐式 1 小时超时的视频、音乐、3D 等长任务
调用方，升级时必须在现有 `PROXY_CONFIG.options` 中显式设置 `"tool_call_timeout": 3600000`，
保留其它配置不变；已显式设置超时的调用方不受默认值变化影响。

`reset_timeout_on_progress` 只在收到有效 progress 通知时延长等待，不能保护不发送 progress 的
长任务。调用超时不代表远端任务未执行或已取消；通用 Proxy 不会因 SDK 超时自动重放当前调用，
调用方应先核对远端状态，避免重复生成或扣费。本地 Maker 已显式设置 1 小时，无需依赖通用默认值。
...
```bash
npm install @taptap/instant-games-open-mcp
...
```bash
npm install -g @taptap/instant-games-open-mcp@latest
```
...
- `tools/list` 只返回 `generate_image`、`batch_generate_images`、`edit_image`、
  ...
  `generate_test_qrcode`、`get_ad_config`、`get_debug_feedbacks`、`achievement`。
...
```bash
# 1. 本地启动 MCP Server
TAPTAP_MCP_TRANSPORT=stdio npx @taptap/instant-games-open-mcp
...
# 文件位置
node_modules/@taptap/instant-games-open-mcp/dist/proxy.js
```
...
```bash
# 克隆仓库
git clone https://github.com/taptap/instant-games-open-mcp.git
cd instant-games-open-mcp
...
- `tenant.custom_fields.session_id` - 业务/编辑器 client session id（可选，业务侧从 `context.customFields?.session_id` 读取）
...
```bash
npm install @taptap/instant-games-open-mcp@latest
cp node_modules/@taptap/instant-games-open-mcp/dist/proxy.js ./proxy.js
```
...
1. **使用原始方式**（需要 `node_modules`）：`npx @taptap/instant-games-open-mcp`
...
│ TapCode 平台服务器                                       │
│                                                         │
│  1. npm install @taptap/instant-games-open-mcp         │
...
interface ProxyConfig {
  ...
  tenant: {
    ...
    custom_fields?: Record<string, string>; // 业务自定义字段；session_id 表示业务/编辑器会话 ID
  };
  auth: {
...
function generateProxyConfig(
  ...
  const config = {
    ...
    tenant: {
      ...
      custom_fields: {
        session_id: editorSessionId, // 业务/编辑器会话 ID
...
function startMCPProxy(user: User, project: Project, macToken: MacToken) {
  ...
  const proxy = spawn(
    'npx',
    ['-y', '-p', '@taptap/instant-games-open-mcp@latest', 'taptap-mcp-proxy', configJson],
    {
