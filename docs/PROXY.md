...
```bash
npm install @taptap/minigame-open-mcp
...
```bash
npm install -g @taptap/minigame-open-mcp@latest
```
...
```json
{
  "mcp.servers": {
    "taptap": {
      ...
      "args": [
        "-y",
        "@taptap/minigame-open-mcp@latest",
        "taptap-mcp-proxy",
...
```bash
# 1. 本地启动 MCP Server
TAPTAP_MCP_TRANSPORT=stdio npx @taptap/minigame-open-mcp
...
# 文件位置
node_modules/@taptap/minigame-open-mcp/dist/proxy.js
```
...
```bash
npm install @taptap/minigame-open-mcp@latest
cp node_modules/@taptap/minigame-open-mcp/dist/proxy.js ./proxy.js
```
...
1. **使用原始方式**（需要 `node_modules`）：`npx @taptap/minigame-open-mcp`
...
│ TapCode 平台服务器                                       │
│                                                         │
│  1. npm install @taptap/minigame-open-mcp         │
...
function startMCPProxy(user: User, project: Project, macToken: MacToken) {
  ...
  const proxy = spawn(
    'npx',
    ['-y', '-p', '@taptap/minigame-open-mcp@latest', 'taptap-mcp-proxy', configJson],
    {
