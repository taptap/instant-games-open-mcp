...
- ❌ **禁止以 `beta` 作为提交到 `main` 的 PR 源分支**。`beta` 仅用于临时汇总
  已验证的开发提交、发布 beta 版本和回归验证，不能在 beta 上直接开发，也不能把
  beta 整体合并回 `main`。
- ✅ 功能必须先保留在对应的 `fix/`、`feature/` 或其它明确的开发分支中；需要发
  beta 时，只能把开发分支中已提交的功能 cherry-pick 到 `beta`。beta 验证完成后，
  必须从原开发分支向目标分支提交 PR，不能从 beta 反向提 PR。
