...
- 统一项目健康检查保持只读，不自动移动、覆盖或重建 `.project` 配置。已有 `.project` 但缺少 `project.json` / `resources.json` 时，先从 Git 或完整错位副本恢复；只有 `settings.json` 仍可解析为 object 时，才在用户确认后补入缺失的 schema/build 默认字段，并保留 `@runtime`、`asset_ignores` 与未知字段。`sources.*.tag` 是锁定字段，只能从完整副本恢复；不要凭默认值生成项目身份、版本、发布元数据或资源分组。`entry=main.lua` 也必须先确认项目实际入口。
- 当前目录是已绑定 Maker 项目时，调用 `generate_test_qrcode` 前必须单独发起一次对话，让用户明确选择横屏（`landscape`）或竖屏（`portrait`）；禁止从代码、截图或已有值推断，也禁止默认横屏。用户选择后传本地私有参数 `confirmed_screen_orientation`；本地 MCP 会在远端调用前校验该值与 `.project/project.json` 的 `taptap_publish.screen_orientation` 一致，并且不会把私有参数转发给远端。缺失或不一致时先修改项目配置并调用 `maker_build_current_directory` 同步，再重试二维码。二维码生成并建立应用身份后，只有用户明确提供 TapTap `user_id` 时才调用 `add_test_whitelist`，不要猜测账号 ID。
