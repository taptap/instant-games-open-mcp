...
被拦截；可重试一次，仍失败再用同一 Maker `node` 直接跑 `__maker-preview-supervisor`。
锁恢复互斥口被代理等临时占用时重试同一 CLI，不结束占用进程。启动失败且控制通道未发布时，
Windows 只在当前 PID 命令行仍是本次 EncodedCommand 时回收包装进程，不按历史 PID 误杀。
