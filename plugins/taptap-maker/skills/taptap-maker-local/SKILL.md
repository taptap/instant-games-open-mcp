...
description: Guide TapTap Maker local development workflows, including the official trigger “打开make mcp控制台”, local web console, local preview, automatic refresh after each related code-edit batch, and Maker achievement management or integration. Use for Maker initialization, clone, project status, console, preview start/refresh/stop and evidence, MCP diagnostics, pull, submit, push, Git conflicts, or achievement/sync/publish/reset requests.
---
...
| achievement / 成就 / 成就管理 / 接入成就 / 发布成就 / 白金成就 | Follow Maker Achievement Workflow below. Do not invent Lua APIs or reuse minigame/H5 achievement SDKs.                                                                                                                |
...
`generate_test_qrcode`, `get_ad_config`, `get_debug_feedbacks`, or `achievement` are missing from the current AI
