...
CLI 负责一次性流程：Git 检查、Python 环境检查、CLI 登录、TapTap token 换取、app 列表选择、
Maker Git clone、AI dev kit 准备、MCP 配置写入与基础验证。Python 环境准备连续 3 次失败时，
...
`taptap-maker doctor` 会检查 Git、Python 环境、PAT、TapTap token、项目绑定、AI dev kit 版本和 MCP 配置。若 Git 不可用，clone/push 会直接停止，直到用户自行安装 Git 并通过 `git --version` 验证。
