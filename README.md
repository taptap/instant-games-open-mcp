...
构建工具的入口和多人配置 schema 与 `maker-tools` build 工具保持同步：
`entry_client` / `entry_server` 会写入 `project.json` 的 `entry@client` /
`entry@server`；`multiplayer.enabled`、`max_players`、`mode`、`background_match`、
`match_info` 和 `persistent_world` 会由远端 build 写入 `.project/settings.json` 的
