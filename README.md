...
taptap-maker logs watch         # CLI，构建成功后内部启动并持续轮询运行日志
```
...
运行时日志不作为本地公开 MCP tool 暴露。构建成功后由 `taptap-maker logs watch` 内部调用
远端 `query_runtime_logs`，默认只拉 `user_script`（客户端 Lua 脚本）和
...
连续失败次数和最后错误。远端返回 `hasMore=true` 时会立即继续拉取，否则每 5 秒轮询一次。
