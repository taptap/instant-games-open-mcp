...
generate_image                  # 远端 proxy 白名单：单图生成
batch_generate_images           # 远端 proxy 白名单：批量生成
edit_image                      # 远端 proxy 白名单：图片编辑
create_video_task               # 远端 proxy 白名单：视频生成任务
text_to_music                   # 远端 proxy 白名单：文本生成音乐
```
...
远端 proxy tools 默认仍作为内部能力隐藏。为试用图片生成链路，Maker MCP 只把
`generate_image`、`batch_generate_images`、`edit_image`、`create_video_task` 和
`text_to_music` 作为白名单公开；这些 tools 的 description、input schema、参数和返回值都
来自远端 server 原始定义，本地不重新封装。
